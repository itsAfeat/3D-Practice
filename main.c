...
void init() {
	...
	/* Load .TIM file */
	tim_texture = (uint32_t*)loadFile("\\STRANGE.TIM;1");
...
void loadMDL(MDLOBJ* obj) {
	/*
		struct ModelData * data = malloc(sizeOfModelFileToLoad);
		readModelFileFromDisc(data);
		unsigned numberOfEntries = sizeOfModelFileToLoad / sizeof(ModelData);
		for (unsigned i = 0; i < numberOfEntries; i++) {
			printf("%x, %x, %x\n", data[i].vertex.x, data[i].vertex.y, data[i].vertex.z);
		}

	*/

	MDLDATA *data = (MDLDATA*)malloc(14);
	data = (MDLDATA*)loadFile("\\CUBE.MDL;1");
